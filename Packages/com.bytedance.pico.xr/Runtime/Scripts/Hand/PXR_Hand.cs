...
namespace ByteDance.PICO.XR
{
    public class PXR_Hand : MonoBehaviour
    {
        ...
        [SerializeField] private Transform rayPose;
        [SerializeField] private GameObject defaultRay;
            ...
            if (PXR_HandTracking.GetJointLocations(handType, ref handJointLocations))
            {
                ...
                for (int i = 0; i < handJoints.Count; ++i)
                {
                    ...
                    else
                    {
                        ...
                        {
                            parentPose = new Pose(handJointLocations.jointLocations[1].pose.Position.ToVector3(),
                                handJointLocations.jointLocations[1].pose.Orientation.ToQuat());
                        }
                        else
                        {
                            parentPose = new Pose(handJointLocations.jointLocations[i - 1].pose.Position.ToVector3(),
                ...
                if (mOffsetPos)
                {
                    Basemesh.localPosition = handJointLocations.jointLocations[(int)HandJoint.JointWrist].pose.Position.ToVector3() +
                                             mOffsetPos.GetHandPosOffset();
                }
