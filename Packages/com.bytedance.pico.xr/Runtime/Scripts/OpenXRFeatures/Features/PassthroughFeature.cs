    ...
    public class PassthroughFeature : OpenXRFeatureBase
    {
...
#if UNITY_EDITOR
        const string k_CompositionLayersPackageId = "com.unity.xr.compositionlayers";
        static AddRequest s_CompositionLayersAddRequest;
        ...
        protected override void GetValidationChecks(List<ValidationRule> rules, BuildTargetGroup targetGroup)
        {
            var AdditionalRules = new ValidationRule[]
            {
                ...
                {
                    message = "Passthrough depends on the com.unity.xr.compositionlayers package.",
                    checkPredicate = () => IsUpmPackagePresent(k_CompositionLayersPackageId),
                    fixItAutomatic = true,
                    fixItMessage = "Add com.unity.xr.compositionlayers to the project via Package Manager.",
                    fixIt = () =>
                    {
                        if (s_CompositionLayersAddRequest == null || s_CompositionLayersAddRequest.IsCompleted)
                        {
                            s_CompositionLayersAddRequest = Client.Add(k_CompositionLayersPackageId);
                        }
                    },
                    error = true
                },
        ...
        static bool IsUpmPackagePresent(string packageId)
        {
            ...
            if (File.Exists(manifestPath))
            {
                ...
                if (manifestText.Contains($"\"{packageId}\""))
                    return true;
            }

            var packagesLockPath = Path.Combine(projectRoot, "Packages", "packages-lock.json");
            if (File.Exists(packagesLockPath))
            {
                var packagesLockText = File.ReadAllText(packagesLockPath);
                if (packagesLockText.Contains($"\"{packageId}\""))
                    return true;
            }
