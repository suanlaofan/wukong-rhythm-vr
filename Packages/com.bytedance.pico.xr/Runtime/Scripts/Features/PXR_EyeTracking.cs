...
namespace ByteDance.PICO.XR
{
    public class PXR_EyeTracking
    {
        ...
        public static bool GetEyeGazePosition(out Vector3 direction)
        {
            direction = Vector3.zero;
            if (!PXR_Manager.Instance.eyeTracking)
                return false;
            ...
            if (!device.TryGetFeatureValue(PXR_Usages.gazePosition, out direction))
            {
                Debug.LogError("PXRLog Failed at GetEyeGazePosition");
                return false;
            }
            return true;
        }
        public static bool GetEyeGazeRotation(out Quaternion rotation)
        {
            ...
            if (!device.TryGetFeatureValue(PXR_Usages.gazeRotation, out rotation))
            {
                Debug.LogError("PXRLog Failed at GetEyeGazeRotation");
                return false;
            }
            return true;
        }
        public static bool GetEyeGazeStatus(out bool status)
        {
            status = false;
            if (!PXR_Manager.Instance.eyeTracking)
                return false;
            ...
            if (!device.TryGetFeatureValue(PXR_Usages.gazeStatus, out status))
            {
                Debug.LogError("PXRLog Failed at GetCombinedEyePoseStatus status");
                return false;
            }
        ...
        public static bool GetEyeGazeTrackingState(out uint state)
        {
            state = 0;
            if (!PXR_Manager.Instance.eyeTracking)
                return false;
            ...
            if (!device.TryGetFeatureValue(PXR_Usages.gazeTrackingState, out state))
            {
                Debug.LogError("PXRLog Failed at GetEyeGazeTrackingState state");
                return false;
            }
            return true;
        }
