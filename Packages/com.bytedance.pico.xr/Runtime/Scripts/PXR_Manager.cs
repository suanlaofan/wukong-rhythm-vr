...
namespace ByteDance.PICO.XR
{
    public class PXR_Manager : MonoBehaviour
    {
        ...
        [HideInInspector]
        public XrFacialSimulationModeBD trackingMode = XrFacialSimulationModeBD.NONE;
        [HideInInspector]
        ...
        public void PollEvent(XrEventDataBuffer eventDB)
        {
            switch (eventDB.type)
            {
                ...
                case XrStructureType.XR_TYPE_EVENT_DATA_SENSE_DATA_UPDATED:
                {
                    ...
                    if (providerHandle == PXR_Plugin.MixedReality.UPxr_GetSenseDataProviderHandle(PxrSenseDataProviderType.LightEstimation))
                    {
                        if (LightEstimationUpdated != null)
                        {
        ...
        private void UpdateMRCCam()
        {
            ...
            if (PLog.LogLevel.LogDebug == PLog.logLevel && null != backgroundCamObj)
            {
                if (mrcCube == null)
                {
        ...
        public void CreateMRCCam()
        {
            ...
            PLog.i(TAG_MRC, "Camera Obj Activated. mrcCamObjActivated : true.");
        }
...
#else
        public static void QuerySpatialMeshAnchor(List<PxrSpatialMeshInfo> meshInfos)
        {
            if (meshInfos!=null)
            {
                for (int i = 0; i < meshInfos.Count; i++)
