...
namespace ByteDance.PICO.XR
{
    public class PXRSample_SpatialAnchor : MonoBehaviour
    {
        ...
        [SerializeField] private Text anchorID;
        [SerializeField] private GameObject savedIcon;
        ...
        private void Update()
        {
            if (uiCanvas.activeSelf)
            {
                uiCanvas.transform.LookAt(
                    new Vector3(uiCanvas.transform.position.x * 2 - Camera.main.transform.position.x,
                        uiCanvas.transform.position.y * 2 - Camera.main.transform.position.y,
                        uiCanvas.transform.position.z * 2 - Camera.main.transform.position.z), Vector3.up);
            }
        }
