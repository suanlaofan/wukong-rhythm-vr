...
using KeyValuePair = ByteDance.PICO.Enterprise.KeyValuePair;
...
namespace ByteDance.PICO.Enterprise
{
    public partial class PXR_EnterprisePlugin
    {
        ...
        private static bool camOpened = false;
        ...
        private static AndroidJavaObject GetEnumType(System.Enum enumType)
        {
            AndroidJavaClass enumjs =
                new AndroidJavaClass("com.pvr.tobservice.enums" + enumType.GetType().ToString().Replace("ByteDance.PICO.Enterprise.", ".PBS_"));
        ...
        public static float UPxr_GetIPD()
        {
            ...
            value = IToBService.Call<float>("pbsGetIPD");
#endif
            return value;
        }
        ...
        private const string PICO_TOBAPI_Lib = "PICO_TOBAPI";

        [DllImport(PICO_TOBAPI_Lib,  CallingConvention = CallingConvention.Cdecl)]
        ...
        #region ET_CameraImage
        [DllImport(PICO_TOBAPI_Lib, CallingConvention = CallingConvention.Cdecl)]
        private static extern int PXR_Et_Init();

        [DllImport(PICO_TOBAPI_Lib, CallingConvention = CallingConvention.Cdecl)]
        private static extern int PXR_Et_StartCapture(PxrEyeFrameCallBack callback);

        [DllImport(PICO_TOBAPI_Lib, CallingConvention = CallingConvention.Cdecl)]
        private static extern int PXR_Et_SetConfig(int targetFps);
        [DllImport(PICO_TOBAPI_Lib, CallingConvention = CallingConvention.Cdecl)]
        private static extern int PXR_Et_StopCapture();

        [DllImport(PICO_TOBAPI_Lib, CallingConvention = CallingConvention.Cdecl)]
        private static extern int PXR_Et_Release();

        [StructLayout(LayoutKind.Sequential)]
        public struct PxrEyeFrame
        {
            public long timestamp;
            public int id; // Camera ID: 15(TL), 16(TR), 17(BL), 18(BR)
            public int width;
            public int height;
            public IntPtr data;
            public override string ToString()
            {
                return string.Format("PxrEyeFrame: timestamp={0}, id={1}, width={2}, height={3}, data={4}", timestamp, id, width, height, data);
            }   
        };

        public delegate void PxrEyeFrameCallBack(ref PxrEyeFrame dataBuffer);
        public static int Et_Init()
        {
#if PICO_PLATFORM
            return PXR_Et_Init();
#else
        ...
        public static int Et_StartCapture(PxrEyeFrameCallBack callback)
        {
#if PICO_PLATFORM
            return PXR_Et_StartCapture(callback);
#else
            return -1;
#endif
        }
        ...
        {
#if PICO_PLATFORM
            return PXR_Et_SetConfig(targetFps);
#else
            return -1;
#endif
        }
