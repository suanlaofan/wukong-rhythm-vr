...
namespace ByteDance.PICO.Enterprise
{
    ...
    public class HandStateAlg
    {
        ...
        {
            // Note: JsonUtility needs to wrap arrays in an object when serializing; here we serialize the root object directly
            return JsonUtility.ToJson(data, true); // When the second parameter is true, output formatted JSON
        }
    }
    [Serializable]
