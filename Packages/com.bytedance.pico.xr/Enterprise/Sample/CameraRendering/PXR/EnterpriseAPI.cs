...
using AOT;
using ByteDance.PICO.Enterprise;
...
{  
    ...
    // Convert video frame to Unity texture
    Texture2D texture;
    public Text fpsText;
    ...
    public void getCameraParameters()
    {
        ...
        CanshuText.text =$"Intrinsics::[fx,fy,cx,cy]=[{param.fx},{param.fy},{param.cx},{param.cy}]\n"+ $"Extrinsics::L=[{param.l_pos},{param.l_rot}]\n"+$" R=[{param.r_pos},{param.r_rot}]";
    }
    ...
    private void Update()
    {
        ...
        if (deltaTime >= showTime) {
            if (count>0)
            {
                ...
                string strFpsInfo = string.Format("Current frame render interval: {0:0.0} ms ({1:0.} frames per second)", milliSecond, fps);
