...
namespace ByteDance.PICO.Platform.Models
{
    ...
    public class CaptureInfo
    {
        ...
        {
            ImagePath = CLIB.CLIB.ppf_CaptureInfo_GetImagePath(o);
            JobId = CLIB.CLIB.ppf_CaptureInfo_GetJobId(o);
        }
    }
    ...
    public class RecordInfo
    {
        ...
        {
            VideoPath = CLIB.CLIB.ppf_RecordInfo_GetVideoPath(o);
            DurationInMilliSeconds = CLIB.CLIB.ppf_RecordInfo_GetDurationInMilliSeconds(o);
            Width = CLIB.CLIB.ppf_RecordInfo_GetWidth(o);
            Height = CLIB.CLIB.ppf_RecordInfo_GetHeight(o);
            JobId = CLIB.CLIB.ppf_RecordInfo_GetJobId(o);
        }
    }
    ...
    public class SessionMedia
    {
        ...
        {
            {
                int sz = (int) CLIB.CLIB.ppf_SessionMedia_GetImagesSize(o);
                Images = new CaptureInfo[sz];
                for (int i = 0; i < sz; i++)
                {
                    Images[i] = new CaptureInfo(CLIB.CLIB.ppf_SessionMedia_GetImages(o, (UIntPtr) i));
                }
            }
            {
                int sz = (int) CLIB.CLIB.ppf_SessionMedia_GetVideosSize(o);
                Videos = new RecordInfo[sz];
                for (int i = 0; i < sz; i++)
                {
                    Videos[i] = new RecordInfo(CLIB.CLIB.ppf_SessionMedia_GetVideos(o, (UIntPtr) i));
                }
            }
        }
    }
