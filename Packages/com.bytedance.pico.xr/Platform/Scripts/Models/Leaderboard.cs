...
namespace ByteDance.PICO.Platform.Models
{
    ...
    public class Leaderboard
    {
        ...
        {
            ApiName = CLIB.CLIB.ppf_Leaderboard_GetApiName(o);
            ID = CLIB.CLIB.ppf_Leaderboard_GetID(o);
            var pointer = CLIB.CLIB.ppf_Leaderboard_GetDestination(o);
    ...
    public class LeaderboardList : MessageArray<Leaderboard>
    {
        ...
        {
            TotalCount = CLIB.CLIB.ppf_LeaderboardArray_GetTotalCount(a);
            NextPageParam = CLIB.CLIB.ppf_LeaderboardArray_HasNextPage(a) ? "true" : string.Empty;
            var count = (int) CLIB.CLIB.ppf_LeaderboardArray_GetSize(a);
            this.Capacity = count;
            for (var i = 0; i < count; i++)
            {
                Add(new Leaderboard(CLIB.CLIB.ppf_LeaderboardArray_GetElement(a, (UIntPtr) i)));
            }
        }
    }
    ...
    public class SupplementaryMetric
    {
        ...
        {
            ID = CLIB.CLIB.ppf_SupplementaryMetric_GetID(o);
            Metric = CLIB.CLIB.ppf_SupplementaryMetric_GetMetric(o);
        }
    }
    ...
    public class LeaderboardEntry
    {
        ...
        {
            DisplayScore = CLIB.CLIB.ppf_LeaderboardEntry_GetDisplayScore(o);
            var extraDataPtr = CLIB.CLIB.ppf_LeaderboardEntry_GetExtraData(o);
            var extraDataSize = CLIB.CLIB.ppf_LeaderboardEntry_GetExtraDataLength(o);
            ExtraData = MarshalUtil.ByteArrayFromNative(extraDataPtr, extraDataSize);
            ID = CLIB.CLIB.ppf_LeaderboardEntry_GetID(o);
            Rank = CLIB.CLIB.ppf_LeaderboardEntry_GetRank(o);
            Score = CLIB.CLIB.ppf_LeaderboardEntry_GetScore(o);
            Timestamp = TimeUtil.SecondsToDateTime((long) CLIB.CLIB.ppf_LeaderboardEntry_GetTimestamp(o));
            User = new User(CLIB.CLIB.ppf_LeaderboardEntry_GetUser(o));
            {
                var pointer = CLIB.CLIB.ppf_LeaderboardEntry_GetSupplementaryMetric(o);
                if (pointer == IntPtr.Zero)
                {
    ...
    public class LeaderboardEntryList : MessageArray<LeaderboardEntry>
    {
        ...
        {
            NextPageParam = CLIB.CLIB.ppf_LeaderboardEntryArray_HasNextPage(a) ? "true" : string.Empty;
            PreviousPageParam = CLIB.CLIB.ppf_LeaderboardEntryArray_HasPreviousPage(a) ? "true" : string.Empty;
            var count = (int) CLIB.CLIB.ppf_LeaderboardEntryArray_GetSize(a); 
            this.Capacity = count;
            for (uint i = 0; i < count; i++)
            {
                this.Add(new LeaderboardEntry(CLIB.CLIB.ppf_LeaderboardEntryArray_GetElement(a, (UIntPtr) i)));
            }

            TotalCount = CLIB.CLIB.ppf_LeaderboardEntryArray_GetTotalCount(a);
        }
    }
}