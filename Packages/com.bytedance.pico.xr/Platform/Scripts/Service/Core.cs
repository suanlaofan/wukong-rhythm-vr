    ...
    public static class CoreService
    {
        ...
        public static Task<PlatformInitializeResult> AsyncInitialize(string appId = null)
        {
            ...
            if (Application.platform == RuntimePlatform.Android)
            {
                ...
                var requestId = CLIB.CLIB.ppf_InitializeAndroidAsynchronous(appId, activity.GetRawObject(), IntPtr.Zero);
                if (requestId == 0)
                {
            ...
            else if ((Application.platform == RuntimePlatform.WindowsPlayer || Application.platform == RuntimePlatform.WindowsEditor))
            {
                ...
                var requestId = CLIB.CLIB.ppf_PcInitAsynchronousWrapper(appId, config.text, logDirectory);
                if (requestId == 0)
                {
                    throw new Exception("PICO PlatformSDK failed to initialize");
                }
        ...
        public static void Initialize(string appId = null)
        {
            ...
            if (Application.platform == RuntimePlatform.Android)
            {
                ...
                initializeResult = CLIB.CLIB.ppf_InitializeAndroid(appId, activity.GetRawObject(), IntPtr.Zero);
            ...
            else if ((Application.platform == RuntimePlatform.WindowsPlayer || Application.platform == RuntimePlatform.WindowsEditor))
            {
                ...
                initializeResult = CLIB.CLIB.ppf_PcInitWrapper(appId, config.text, logDirectory);
                if (initializeResult == PlatformInitializeResult.Success ||
                    initializeResult == PlatformInitializeResult.AlreadyInitialized)
                {
                    Initialized = true;
        ...
        public static Task<GameInitializeResult> GameInitialize(string accessToken)
        {
            if (Initialized)
            {
                return new Task<GameInitializeResult>(CLIB.CLIB.ppf_Game_InitializeWithToken(accessToken));
            }
        ...
        public static Task<GameInitializeResult> GameInitialize()
        {
            if (Initialized)
            {
                return new Task<GameInitializeResult>(CLIB.CLIB.ppf_Game_InitializeAuto());
            }
        ...
        public static bool GameUninitialize()
        {
            if (Initialized)
            {
                return CLIB.CLIB.ppf_Game_UnInitialize();
            }
