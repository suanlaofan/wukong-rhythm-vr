...
namespace ByteDance.PICO.Platform.Service
{
    ...
    public static class NetworkService
    {
        ...
        public static Packet ReadPacket()
        {
            ...
            var handle = CLIB.CLIB.ppf_Net_ReadPacket();
        ...
        public static bool SendPacket(string userId, byte[] bytes)
        {
            ...
            var ok = CLIB.CLIB.ppf_Net_SendPacket(userId, (UIntPtr) bytes.Length, pobj);
            if (hobj.IsAllocated)
                hobj.Free();
            return ok;
        }
        ...
        public static bool SendPacket(string userId, byte[] bytes, bool reliable)
        {
            ...
            GCHandle hobj = GCHandle.Alloc(bytes, GCHandleType.Pinned);
            IntPtr pobj = hobj.AddrOfPinnedObject();
            var ok = CLIB.CLIB.ppf_Net_SendPacket2(userId, (UIntPtr) bytes.Length, pobj, reliable);
            if (hobj.IsAllocated)
            {
                hobj.Free();
            }
        ...
        public static bool SendPacketToCurrentRoom(byte[] bytes)
        {
            ...
            GCHandle hobj = GCHandle.Alloc(bytes, GCHandleType.Pinned);
            IntPtr pobj = hobj.AddrOfPinnedObject();
            var ok = CLIB.CLIB.ppf_Net_SendPacketToCurrentRoom((UIntPtr) bytes.Length, pobj);
            if (hobj.IsAllocated)
            {
                hobj.Free();
            }
        ...
        public static bool SendPacketToCurrentRoom(byte[] bytes, bool reliable)
        {
            ...
            GCHandle hobj = GCHandle.Alloc(bytes, GCHandleType.Pinned);
            IntPtr pobj = hobj.AddrOfPinnedObject();
            var ok = CLIB.CLIB.ppf_Net_SendPacketToCurrentRoom2((UIntPtr) bytes.Length, pobj, reliable);
            if (hobj.IsAllocated)
                hobj.Free();
            return ok;
        }
        ...
        /// * If you are in a room before receiving this message, you will need to check and reset your room state after receiving this message.
