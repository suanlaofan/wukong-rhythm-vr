...
namespace ByteDance.PICO.Platform.Editor
{
    public class PlatformManifestRewrite : IPostGenerateGradleAndroidProject
    {
        ...
        public void OnPostGenerateGradleAndroidProject(string path)
        {
            ...
            // Remove existing meta-data if present to avoid duplication
            for (int i = app.ChildNodes.Count - 1; i >= 0; i--)
            {
                var child = app.ChildNodes[i] as XmlElement;
                if (child != null && child.Name == "meta-data")
                {
                    string name = child.GetAttribute("name", androidUri);
                    if (string.IsNullOrEmpty(name) && child.Attributes["android:name"] != null)
                    {
                        name = child.Attributes["android:name"].Value;
                    }
                    
                    if (name == "pvr.app.id" || name == "use_record_highlight_feature")
                    {
