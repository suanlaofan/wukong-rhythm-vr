...
namespace ByteDance.PICO.Platform.Editor
{
    public class PicoSettings : EditorWindow
    {
        ...
        static string strAppIdText = "Paste your App ID here";
        static string strAppIdHelpText = "App ID is the unique identification ID of the PICO Application. Without AppID, you will not be able to use PICO platform feature.";
        static string strBuildSettingText = "Recommend Settings [?]";
        static string strBuildSettingHelpText = "Recommended project settings for PXR SDK";
        static string strPlatformBuildText = "Set Platform To Android";
        static string strUnityVersionLimit = $"Unity Editor Version ≥ {EditorConf.minEditorVersion}";
        static string strOrientationBuildText = "Set Orientation To LandscapeLeft";
        static string strMinApiLevel = $"Android Min API Level ≥ {EditorConf.minSdkLevel}";
        static string strIgnoreButtonText = "Ask me later";
        static string strApplyButtonText = "Apply";
        static string strHighlightText = "Use Highlight";
        ...
        void OnGUI()
        {
            ...
            {
                GUILayout.Space(15);
                GUILayout.Label(strAppIdText);
                var newAppId = EditorGUILayout.TextField(appId, GUILayout.Width(frameWidth));
                if (newAppId != appId)
                {
                    appId = newAppId;
                }
                ...
                {
                    EditorGUILayout.HelpBox(strAppIdHelpText, UnityEditor.MessageType.Warning);
                }
            ...
            //Highlight settings
            {
                ...
                GUILayout.Label(strHighlightText);
                var newUseHighlight = EditorGUILayout.Toggle(useHighlight, GUILayout.Width(frameWidth));
                if (newUseHighlight != useHighlight)
                {
                    useHighlight = newUseHighlight;
                }
            ...
            {
                GUILayout.Space(5);
                GUILayout.Label(new GUIContent(strBuildSettingText, strBuildSettingHelpText));
            ...
            //Button area
            {
                ...
                if (hasSomethingToFix)
                {
                    ...
                    if (GUILayout.Button(strIgnoreButtonText, GUILayout.Width(130)))
                    {
                        this.Close();
                    }

                    GUI.enabled = hasSomethingToFix;
                    if (GUILayout.Button(strApplyButtonText, GUILayout.Width(130)))
                    {
