...
namespace ByteDance.PICO.XR.Editor
{
    ...
    public class PXR_ARCameraEffectManagerEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            if (manager.enableCameraEffect)
            {
                ...
                manager.lutTex1 = (Texture2D)EditorGUI.ObjectField(new Rect(textureControlRect.x, textureControlRect.y, 100, textureControlRect.height),
                    manager.lutTex1, typeof(Texture), false);
                ValidateTexture(manager.lutTex1);

                manager.lutTex2 =
                    (Texture2D)EditorGUI.ObjectField(
                        new Rect(textureControlRect.x + textureControlRect.width, textureControlRect.y, textureControlRect.width, textureControlRect.height),
                        manager.lutTex2, typeof(Texture), false);
                ValidateTexture(manager.lutTex2);

                manager.lutTex3 =
                    (Texture2D)EditorGUI.ObjectField(
                        new Rect(textureControlRect.x + 2 * textureControlRect.width, textureControlRect.y, textureControlRect.width,
                            textureControlRect.height), manager.lutTex3, typeof(Texture), false);
                ValidateTexture(manager.lutTex3);

                manager.lutTex4 =
                    (Texture2D)EditorGUI.ObjectField(
                        new Rect(textureControlRect.x + 3 * textureControlRect.width, textureControlRect.y, textureControlRect.width,
                            textureControlRect.height), manager.lutTex4, typeof(Texture), false);
                ValidateTexture(manager.lutTex4);

                manager.lutTex5 =
                    (Texture2D)EditorGUI.ObjectField(
                        new Rect(textureControlRect.x + 4 * textureControlRect.width, textureControlRect.y, textureControlRect.width,
                            textureControlRect.height), manager.lutTex5, typeof(Texture), false);
